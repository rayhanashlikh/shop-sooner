...
$routes->group('/', ['filter' => 'user'] , function ($routes) {
    ...
    $routes->group('cart', function ($routes){
        ...
        $routes->get('cek', 'Cart::cek');
        $routes->post('add', 'Cart::add');
        $routes->get('clear', 'Cart::clear');
        $routes->post('update', 'Cart::update');
        $routes->post('delete', 'Cart::delete');
    });

    $routes->group('order', function ($routes) {
        $routes->get('/', 'Order::index');
        $routes->post('checkout', 'Order::checkout');
        $routes->get('confirm-checkout/(:segment)', 'Order::confirm_checkout/$1');
        $routes->get('get-kota', 'Order::action');
        $routes->post('get-courier-services', 'Order::getCourierServices');
        $routes->patch('confirm-order/(:segment)', 'Order::confirm/$1');
    });
});
